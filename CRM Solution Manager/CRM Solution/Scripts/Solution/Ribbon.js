  ...
  SYED.Solution.Ribbon = {
        ...
        if (selectedId.length > 0) {
          ...
          Xrm.WebApi.online.retrieveRecord('solution', selectedId, '?$select=ismanaged').then(
            function success(result) {
              const ismanaged = result.ismanaged;
              if (ismanaged) {
    ...
    CallAction: function(selectedId, mode) {
      try {
        ...
        req.open(
          'POST',
          Xrm.Page.context.getClientUrl() + '/api/data/v9.1/syed_CreateDynamicsSourceControlBySolution',
          false,
        );
          ...
          if (this.readyState === 4) {
            ...
            if (this.status === 200) {
              ...
              Xrm.Utility.alertDialog(
                mode + '- is in progress, for more details, Please refer Dynamics Source Control record',
              );
            } else {
